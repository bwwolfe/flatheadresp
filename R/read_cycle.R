...
#'
#' AquaResp stores each experimental cycle (i.e. the measurement period when MO2 is calculated as a seperate file.
#' This function reads the data from a specific cycle file in the "All slopes" folder of an AquaResp experiment.
#'
...
#'
#' Calculates mass-specific oxygen consumption (MO2) for each chamber in a given cycle
#' using linear regression on PO2 data and experimental metadata.
#'
#' @param cycle_number Numeric. The cycle number to calculate MO2 for.
#' @param path Character. The path to the AquaResp experiment directory.
#'
#' @return A named numeric vector of MO2 values for each chamber.
#' @export
#'
#' @examples
#' \dontrun{
#' calc_cycle_mo2s(cycle_number = 1, path = "~/data/Resp/Experiment_1")
#' }
calc_cycle_mo2s <-
  function(cycle_number, path) {
...
#'
#' Extracts the minimum and maximum non-zero PO2 values for each chamber from a cycle dataframe.
#'
#' @param cycle Dataframe. A cycle dataframe as returned by `read_cycle()`.
#'
#' @return A named numeric vector of min and max PO2 values for each chamber.
#' @export
#'
#' @examples
#' \dontrun{
#' cycle <- read_cycle(1, "~/data/Resp/Experiment_1")
...
#'
#' Computes the percentage of zero values in each PO2 column of a cycle dataframe.
#' Values greater than zero would have an incorrect R^2 as calculated by AquaResp.
#'
#' @param cycle Dataframe. A cycle dataframe as returned by `read_cycle()`.
#'
...
#'
#' Performs Pearson correlation tests between PO2 values and Unix time for each chamber in a cycle.
#'
#' @param cycle Dataframe. A cycle dataframe as returned by `read_cycle()`.
#'
...
get_cycle_correlations <-
  function(cycle) {
    ...
    r2s <- lapply(cors, function(x)
      x$estimate) |>
      stats::setNames(paste0(names(cycle[po2_cols]),
                      ".r2"))
...
# output is a dataframe with columns for the following variables:
#
#   min po2
#   max po2
#   slope r2
#   slope r2 p value
#   percent 0s in the cycle (%)
#
# separate columns for each chamber (i.e., 5 columns for each of ch1, ch2, ...)

#' Summarize PO2 metrics across all cycles in an experiment
#'
#' Generates a summary dataframe with PO2 statistics (min, max, correlation, missingness)
#' for each cycle and chamber in the experiment. The correlations are calculated with
#' missing values (po2 = 0) omitted to get around the bug in AquaResp.
#'
#' @param path Character. The path to the AquaResp experiment directory.
#'
#' @return A dataframe with one row per cycle and columns for each PO2 metric for each chamber.
#' @export
#'
#' @examples
#' \dontrun{
#' get_exp_cycle_summary(path = "~/data/Resp/Experiment_1")
#' }
...
#'
#' Generates a scatter plot of PO2 values over time for each chamber in a given cycle.
#'
#' @param cycle_number Numeric. The cycle number to plot.
#' @param path Character. The path to the AquaResp experiment directory.
#'
#' @return A base-R plot of time vs PO2.
#' @export
#'
#' @examples
#' \dontrun{
#' plot_cycle_po2(cycle_number = 1, path = "~/data/Resp/Experiment_1")
#' }
